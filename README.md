...
> **Note on data:** The entire `data/` directory is excluded from this repository via `.gitignore`. This includes `synthea.jar` (~188 MB, exceeds GitHub's 100 MB limit) and all generated CSV files. See the [Data Setup](#data-setup) section for full instructions.
...
│   ├── synthea.jar                #   download separately (~188 MB)
│   └── synthea_raw/csv/           #   generated by Synthea
│
├── requirements.txt
└── README.md
```
...
- Python 3.11+
- Java 11+ (for Synthea data generation — one-time only)
- cmake + ninja (required to compile liboqs)
...
```bash
sudo apt-get install cmake ninja-build libssl-dev default-jre
```
...
```bash
git clone https://github.com/OmGandhi091611/TreeMedChain.git
cd TreeMedChain
```
...
`liboqs-python` will compile the liboqs C library from source automatically during this step. It requires cmake and ninja (installed above). This takes 2–5 minutes on first install.

---

## Data Setup

The `data/` directory is not included in the repository. Follow these steps to set it up.
...
Synthea is a synthetic patient generator by MITRE. The JAR is ~188 MB and must be downloaded separately.
...
Or download `synthea-with-dependencies.jar` manually from https://github.com/synthetichealth/synthea/releases and place it at `data/synthea.jar`.

### Step 3 — Generate 573 synthetic patients
...
This generates `data/synthea_raw/csv/` with the following files used by the loader:

| File | Contents |
|------|----------|
| `patients.csv` | Demographics, gender, DOB |
| `conditions.csv` | Diagnoses (SNOMED codes) |
| `medications.csv` | Prescriptions |
| `procedures.csv` | Procedures |
| `observations.csv` | Lab results and vitals |

> Generation takes 1–3 minutes. The `data/` directory is in `.gitignore` and will never be committed.

### Verify the data loaded correctly

```bash
venv/bin/python src/synthea_loader.py data/synthea_raw/csv
```

Expected output: `Loaded 573 patient(s)` followed by a sample tree printout.

---

## Running Tests

Most tests do not require Synthea data — they use small in-memory fixtures. Only `test_synthea_loader.py` needs the generated CSVs.
...
### Tests that do NOT need Synthea data

These run immediately with no data setup required:
...
### Tests that require Synthea data

Complete the [Data Setup](#data-setup) steps first, then:
...
```
46 passed in ~1-2 minutes
```

The PQC tests (`test_pqc.py`) dominate runtime because Kyber and Dilithium key generation is computationally intensive. This is expected behavior — these are production-grade post-quantum algorithms.

---

## Running Experiments

Experiments require Synthea data. Complete [Data Setup](#data-setup) first.

### All experiments + all figures (recommended)

```bash
venv/bin/python -m experiments.run_all --synthea-dir data/synthea_raw/csv
```

This runs all three experiments in sequence then generates all four figures. Estimated runtime: **15–25 minutes**.

### Individual experiments

```bash
# Privacy accuracy: naive vs. TreeMedChain across roles, systems, patient counts
venv/bin/python -m experiments.exp_privacy --synthea-dir data/synthea_raw/csv

# Crypto + blockchain overhead benchmarks (no Synthea data needed)
venv/bin/python -m experiments.exp_overhead

# Scalability: end-to-end access time vs. patient count
venv/bin/python -m experiments.exp_scalability --synthea-dir data/synthea_raw/csv

# Regenerate all figures from existing CSVs without re-running experiments
venv/bin/python -m experiments.plot_all
```

### Regenerate figures only (experiments already run)
...
All results are saved to `experiments/results/` (also excluded from git):
...
| `plots/fig1_privacy_accuracy.png` | Adversary accuracy curves — main paper result |
